 ...
 * @param[in] params Creation parameters. Pass NULL to use defaults.
 * @param[out] payload Pointer to memory where the created payload handle will be written to.
 *
 * @returns an error code on failure else \ref VPI_SUCCESS
 */
VPI_PUBLIC VPIStatus vpiCreateStereoDisparityEstimator(uint32_t backends, int32_t imageWidth, int32_t imageHeight,
 ...
 * Please refer to \ref algo_stereo_limitations for information on limitations for
 * some parameters.
 *
 ...
 * @param[out] confidenceMap Currently it must be NULL as the algorithm does not support confidence map output.
 * @param[in] params Pointer to algorithm control parameters. If NULL, windowSize==5 and maxDisparity is the same as passed during payload creation.
 *
 * @returns an error code on failure else \ref VPI_SUCCESS
 */
VPI_PUBLIC VPIStatus vpiSubmitStereoDisparityEstimator(VPIStream stream, uint32_t backend, VPIPayload payload,
